  ...
  const handleBooking = async () => {
    try {
      ...
      const response = await axios.post(
        "http://localhost:8000/api/booking-detail",
        formData
      );
  ...
  return (
    <div>
      <header className="header-room-detail">
        <a href="#">
          <img src="logo.png" className="logo-room-detail" alt="Logo" />
        </a>
      </header>
      ...
      <section id="roomdetails-room-detail" className="section-p1-room-detail">
        <div className="single-room-image-room-detail">
          ...
          <div className="small-img-group-room-detail">
            {[
              ...
              <div
                key={index}
                className="small-img-col-room-detail"
        ...
        <div className="single-room-details-room-detail">
          ...
          <h4>{roomDetails.name || "Deluxe King Room"}</h4>
          <h2>${roomDetails.price || "200"} per night</h2>
          ...
          <h4>Room Details</h4>
          <span>
            {roomDetails.description || "This Deluxe King Room offers a luxurious experience with scenic views. Enjoy a relaxing stay with top-notch amenities and an elegant design that ensures comfort and style."}
          </span>
        </div>
      </section>
    </div>
